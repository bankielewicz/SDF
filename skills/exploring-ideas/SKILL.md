...
**5. Write the brief and the seed data.** Write `.devforgeai/explore/brief.md` from `templates/brief.md` with all twelve sections filled and `status: specified`, and `.devforgeai/explore/seed-data.json` from the `seed_data` field in the shape `references/brief-sections.md` fixes — the seven envelope keys at the top level beside `entities`, in one flat object. `references/brief-sections.md` gives the content rule for each brief section and that shape. On a remedy run only the cited rows of `## Core flows`, the frontmatter `open_questions`, and `status` change. The PostToolUse hook runs `devforgeai doc validate` on the written path and its result reaches the model after the write as `hookSpecificOutput.additionalContext`; rewrite the section it names.
...
| Seed data | `.devforgeai/explore/seed-data.json` | shape in `references/brief-sections.md` |
