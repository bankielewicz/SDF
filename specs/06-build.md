...
**4. Open the worktree — CLI.** Input: the `STORY-nnn` from step 1. Actor: the model runs `devforgeai worktree ensure <STORY-nnn>`, which prints the worktree path on stdout. Output: a worktree at `<[build].worktree_root>/<STORY-nnn>` on branch `<[build].branch_prefix><STORY-nnn>`, carrying its own `.devforgeai/state.toml` seeded from the main checkout, and the path the rest of the run works in. Failure path: exit 1 on `DFA-E272`, meaning another worktree under `worktree_root` holds a story whose `## Files` set intersects this one; the run stops with one `Blocked` line naming both story ids and the shared path. Exit 1 on `DFA-E271`, meaning the project root is not a git work tree; the run stops with one `Blocked` line naming `git init`.

**5. Activate the phase — CLI.** Input: the `STORY-nnn`. Actor: the model runs `devforgeai phase set build --id <STORY-nnn>` inside the worktree. Output: `state.toml` `[current].phase` of `build`, `[current].id` and `[active].build` of the story id, and the story file's frontmatter `status` moved to `building` by the CLI. From this point the `PreToolUse` `story files --check` hook resolves `--id` from `[active].build` and guards every write. Failure path: exit 1 on `DFA-E320`, meaning the plan gate is not PASS; the run stops and the Stop hook prints the gate result.
...
14. Proposed addition to `specs/01-cli.md` §Outputs, the `state.toml` section: `.devforgeai/state.toml` is untracked and every other path under `.devforgeai/` is tracked. This is what makes a worktree work: `--project` resolves to the worktree because `.devforgeai/` is checked out there, each worktree keeps its own `[active].build` so the `PreToolUse` `story files --check` guard resolves the right story, and two concurrent builds produce no merge conflict on a state file. `worktree ensure` copies the main checkout's `state.toml` into the new worktree, because a `phase set` in a worktree with no state file would start from the default `explore`.
