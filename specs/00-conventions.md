...
| `stack detect` | Write `config.toml` with detected languages, test/coverage/lint commands, source roots. | `init`, SessionStart hook |
...
|---|---|---|---|---|---|
| SessionStart | `startup\|resume\|clear\|fork` | `session-start` | `stack detect`, `handoff` | plain stdout, which this event adds to Claude's context | no channel; a trust failure emits `systemMessage` |
...
- Every hook runs `devforgeai trust verify` first. A trust failure blocks on `PreToolUse`, `Stop`, `SubagentStop`, and `UserPromptExpansion`, through exit 2 and a decision object; on `SessionStart` and `PostToolUse` no exit code blocks and the hook emits `systemMessage` so the failure is visible. `[last_gate].result` takes `TRUST_FAIL`, which is a record rather than the channel the refusal depends on — the binary under suspicion is the one that renders the handoff.
...
2. `evals/cases.jsonl`: one JSON object per line, `{ "id": str, "prompt": str, "setup": {"files": {path: content}, "git": bool | {"worktrees": [{"path": str, "branch": str}]}}, "answers": {question header: label | [label]}, "expect": {"grader": str, "args": {}} }`. At least 6 cases. At least 2 exercise the SEND BACK path. `answers` pre-seeds the skill's questions; `setup.git` makes the workspace a git work tree for a case whose subject needs one.
