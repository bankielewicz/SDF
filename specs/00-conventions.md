...
An id is *defined* by a document's frontmatter `id`, an id opening a Markdown heading, a list item opening `<PREFIX>-<nnn>:`, a YAML sequence item's `id:` key, or a Markdown table row's cell in a column headed `ID` — which is how `## Core flows` defines its `FLOW-nnn` ids, since nothing else in the brief states them. The header scopes it: `## Mockups` leads with a `FLOW-nnn` too, under a different header, and counting that would make every flow a duplicate. Every other occurrence is a reference, and `PREFIX-000` is neither: it is the placeholder a template carries for an example entry (`ADR-000`, `STORY-000`), so it is never indexed and never reported unresolved. Real ids start at `001`.
