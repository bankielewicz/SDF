...
**4. Assemble the release set — CLI, model.** Input: the preamble's stdout, which is `devforgeai story list --status built --json`. The model reads each listed `stories/STORY-nnn.md` for its first H1, its frontmatter `consumes`, and its `status`, then reads `stories/sprint.yaml` `stories[]` and records any `{id, status}` pair whose status is `built` and whose id the CLI list omits. Output: the release set, ascending by id, each with `title`, `requirements`, and the two report paths. Failure path: the set is empty; the run stops with `Blocked   you: no story is at status built; run /verify <STORY-nnn> first` and writes no release file.
...
**12. Move to released — model.** Input: the file from step 11. Output: the same file with `status: released` and `released_at` at the current UTC instant. Failure path: none; the gate at step 13 is what decides whether the version stands, and a FAIL leaves the file at `released` with `signoff.gate` carrying the verdict.

**13. Run the gate — CLI.** Input: `devforgeai gate check --phase release --id $1`. Output: `.devforgeai/reports/$1-release.yaml` with the eight check entries and the ingested `verifiers.deferrals` block, and `signoff` filled from it. Exit 0 goes to step 14. Exit 2 goes to `## Send-back`. Exit 1 leaves the file at `status: released` with `signoff.gate: FAIL` and the run stops. Failure path: exit 1 on `DFA-E303`, meaning `gates.toml` sits below a compiled floor; the run stops and the stderr line reaches the model.
...
| `devforgeai gate check --phase release --id <vX.Y.Z>` | workflow step 13, and the CI job `devforgeai-release-gate` | 0 goes to step 14; 2 goes to `## Send-back`; 1 stops the run with `signoff.gate: FAIL` |
...
Release sends back to Verify only. The destination is the gate's `send_back_to`, and three checks resolve `on_fail = "send_back"`.
