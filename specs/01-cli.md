...
[[worktree]]                                   # zero or more; appended by `worktree ensure`, deleted by `worktree remove`, read by `worktree list`
story = "STORY-014"                            # string; STORY-nnn; the story this worktree was opened for
path = "../wt/STORY-014"                       # string; the worktree directory relative to the project root
branch = "story/STORY-014"                     # string; the branch it was created on
created_at = "2026-09-11T09:14:02Z"            # string; RFC 3339 UTC
...
`.devforgeai/state.toml` is the one path under `.devforgeai/` that git does not track; `init` adds it to `.gitignore`.

**There is one state file, and it lives in the main checkout.** A Build worktree does not carry a copy. `worktree ensure` registers the worktree in the root's state as one `[[worktree]]` entry — `story`, `path` relative to the project root, `branch`, and `created_at` — appending it, idempotent per story so a re-run for an open story adds nothing. `worktree remove` deletes the entry and `worktree list` reads the array. Project discovery inside a linked worktree resolves `.devforgeai/` at the main checkout, so a command run from the worktree reads and writes the same state as one run from the root. The test is two `git rev-parse` calls: in the main checkout `--git-dir` and `--git-common-dir` name the same directory, and in a linked worktree the first is `<main>/.git/worktrees/<name>` while the second is `<main>/.git`. A directory holding no `state.toml` of its own whose two answers differ is a linked worktree, and discovery redirects to the common directory's parent.

`ensure` deletes a `state.toml` it finds in the worktree. Git checks one out whenever a project's `.gitignore` never learned about the file and it was committed, and a file there would be read in preference to the root's by the very test above — which is the failure the one-file rule exists to remove.

The earlier shape copied `state.toml` into each worktree so two concurrent builds could hold different `[active].build` values. That produced two writers for one phase: the Stop hook fires in the session's own directory, which is the root, while the work happened in the worktree, so the gate read the root's copy and the build wrote the worktree's, and whichever the user last looked at was wrong. An array in one file keeps the concurrency that shape was reaching for — several worktrees may be registered at once, on different branches over different source trees, and `worktree ensure` still refuses an overlapping file set with `DFA-E272` — and leaves one answer to which story is being built.

**Which root a command reads.** `Ctx::source_root()` is the `path` of the `[[worktree]]` entry whose `story` equals `[active].build`, and the project root when no entry matches. It is what the command-running and source-path gate kinds use, along with `story files --check`, `antipattern scan`, `commit`, and the `PreToolUse` scope checks. Documents are always read from the project root.

Two arms resolve a story from the worktree rather than from `[active].build`, which is what keeps parallel builds working. The `PreToolUse` `story files --check` arm takes the path being written and, when it lies under a registered worktree, checks it against that entry's `story` — so a write in `../wt/STORY-017` is judged against STORY-017's declared file set even while `[active].build` names STORY-014. And `phase set build` run from inside a registered worktree sets `[active].build` to that worktree's story, so entering a second build from its own directory does the obvious thing.

So a Stop hook fired in the root evaluates the build gate's `tests_pass`, `coverage_min`, `lint_clean` and `complexity_clean` against the worktree of `[active].build` and its `doc_valid` and id checks against the root's documents, which is the split the phase actually has.

The phase-scoped tables are `[explore]`, `[constitute]`, and `[plan]`. The other four phase names are reserved as table names and hold no key in v1. `[[worktree]]` is an array rather than a phase table, because a project may hold several open at once. A key in a phase table that this schema does not list is `DFA-E108`, so a typo fails rather than being silently unread.
...
The two id columns govern different things and are read separately. `ID prefixes` is the set of prefixes a document of that type may *define* in its body; the `id` grammar alone governs the document's own id, and the two need not intersect. A `qa-report` is the clearest case: its own id is a `STORY-nnn`, because a QA report is about a story, while the ids it defines are `FIND-nnn`. Reading the prefix list as a constraint on the document's own id would refuse that file.
...
For a Markdown document: line 1 is exactly `---`; the block ends at the next line that is exactly `---`; the text between is a YAML mapping. For a YAML document: the same keys are top-level keys of the document, and a leading `---` document marker is permitted and ignored. For a JSON document the keys are top-level keys of the root object, exactly as for YAML, with one exception: `brand/tokens.json` carries them under the top-level object key `meta`.

That exception is the only one, and it is enforced rather than merely documented. Every JSON document kind but `tokens` — `explore-payload`, which covers `explore/seed-data.json` and `explore/sketch-request.json` — requires the seven envelope keys at the root and refuses a `meta` wrapper: a root object carrying `meta` and no `schema` of its own is `DFA-E203`, the key-absent code, naming `schema` with the message `<path> frontmatter has no key 'schema' at the top level; \`meta\` is not a wrapper for this document`. The `tokens` row is the only one whose frontmatter source is `JsonMeta`; every other JSON kind reads the root object, so a wrapper there leaves the validator seeing no `schema` at all.

`tokens` is wrapped for a reason that does not generalise: the rest of that file is a token tree a design tool reads, and a `schema` key at the root would be read as a token group named `schema`. No other JSON document in the framework has a foreign reader, so no other one pays that cost — and a second wrapped kind would make the unwrapped case the exception, which is the direction a validator cannot follow.
...
| `id` | string | matches the `id` grammar of the doc-type row |
...
`--json` `data` for `ensure`: `{"id":"STORY-014","path":"../wt/STORY-014","branch":"story/STORY-014","created":true,"registered":true}`. For `list`: `{"worktrees":[{"id":"STORY-014","path":"../wt/STORY-014","branch":"story/STORY-014","dirty":false,"ahead":3}]}`. For `remove`: `{"id":"STORY-014","removed":true,"branch_deleted":true}`.
...
**The Stop hook and a Build worktree.** The hook fires in the session's own directory, which is the main checkout, and the work of a Build run happened in the worktree. Both are reached from the one state file: the gate's document and id checks read `.devforgeai/` at the root, and its command-running and source-path checks run against `Ctx::source_root()`, the `path` of the `[[worktree]]` entry whose `story` is `[active].build`. Nothing is copied and nothing is re-rooted; the hook needs no knowledge of where the session is, because the state says where the sources are.
...
**Which root the engine resolves a path against.** Every check in this file runs against one of two roots, and the split is by what the path names rather than by which gate holds the check. Source resolves against `Ctx::source_root()`, the registered worktree of `[active].build` and the project root when none is registered: the commands `tests_pass`, `coverage_min`, `lint_clean`, `complexity_clean` and `docs_cover` spawn, their working directory, the coverage artifact `coverage_paths` names, the `docs.api[]` pages `docs_cover` reads, the deploy manifests `deploy_manifest` reads, and any check path that leads with a dot. Documents resolve against the main checkout and never move: everything under `.devforgeai/`, which is `state.toml`, `config.toml`, `gates.toml`, every phase document a `document` or `path` key names, and every report. The five delegating kinds — `design_tokens`, `antipattern_clean`, `story_valid`, `context_audit`, `files_declared` — inherit the same split through the `cmd/` functions they call, rather than restating it.

The reason is that a Build run's sources are in the worktree and its documents are not. A gate that resolved everything against one root would either run the test command in a checkout the work is not in, or look for the story document in a worktree that does not carry `.devforgeai/`. Both were observed before the split was made explicit.
...
`@@NOW@@` is replaced with the RFC 3339 UTC timestamp at write time. `[stop_hook].blocked_session`, `[stop_hook].scanned_at`, and the whole of `[plan]`, `[[worktree]]`, and `[last_cross]` are absent from the initial file and take their empty defaults on load, so a state file written before those keys existed still parses.
...
73. `.devforgeai/state.toml` is untracked and every other path under `.devforgeai/` is tracked. `init` writes that line and `.explore-prototype/` into `.gitignore`. The file exists only in the main checkout: each open worktree is one `[[worktree]]` entry in it rather than a copy of it, because two copies gave the Stop hook and the build two different answers to which story was being built. The array is what preserves parallel builds, and `worktree ensure` still refuses an overlapping file set with `DFA-E272`.
