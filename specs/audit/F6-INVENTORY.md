...
## Ninth pass — F2's batch landed (959 tests)

1. **Verifier registry.** `flow-integrity-auditor` moves to `required = true`, and the prose that listed it among the verifiers no `verifier_pass` names now excludes it and says why: the discover gate's `flow-integrity` check reads its block, so an absent one is `DFA-E316`. The same paragraph records that the check carries no `unit` key, because `verifier_pass` takes `verifiers` and `min_ratio` and nothing else — the `unit` column is what the agent writes in its envelope, not a key a check reads. The check count was already sixty-four and matches `cli/tests/default_gates.rs:69`.

   Syncing `03-discover.md`'s fence to F2's landed bytes caught **two drifts my eighth pass had not**: the check I wrote in house style was `id = "discover-flows"` where F2 landed `id = "flow-integrity"` with a `severity` and a `message`, and the `accepted` check still read `path = "accepted_by"` where `01-cli` has read `field` since F2's first-wave edit. Both fixed; the discover gate block is now byte-identical between the two specs. The `## Send-back` prose, which said "only at step 4", now gives both points.

2. **ID index.** Read from `cli/src/doc/ids.rs:264-295`: a table row defines only in a column whose header is `ID`, and the scoping is load-bearing rather than tidiness — `## Mockups` also leads with a `FLOW-nnn`, under a different header, and counting it would make every flow a duplicate and raise `DFA-E209`. `PREFIX-000` is neither defined nor referenced. `DFA-W202` skips ids the document defines itself, since `consumes` is for what a document takes from elsewhere. All three in `01-cli` and the first two in conventions §5.

3. **`story validate`.** The `DFA-E412` widening from my eighth pass is reverted; that column now reads `report ingest, story files --check`. The `story validate` section states the new behaviour — no `<id>` and no `[active].build` is `DFA-E011` at exit 3, a usage error rather than the exit-0 pass it used to report — and says why `story files --check` keeps its exit-0 `DFA-E412`: it runs from the `PreToolUse` hook on every write, and a non-zero there would block every edit made outside a Build run. `DFA-E011` added to the section's exit-code line.

Both docs facts reproduced live against the rebuilt binary: `Copied     9 skills (102 files), 46 agents`, and `story validate` with no subject printing `DFA-E011 ... requires --id, or an active build story in state.toml` at exit 3. The guide's row was corrected from my eighth-pass wording, which had guessed a non-zero `DFA-E412`.

## Tenth pass — one state file, `[[worktree]]`; the skill-name collision

**1. Worktree registration.** Written to the coordinator's refinement, which supersedes the single-field `[build]` shape F2 had already landed (`cli/src/state.rs:245-248`, `BUILD_KEYS` at `:398`). The `01-cli` paragraph carries a `<!-- verify against cli/src/state.rs -->` marker naming that discrepancy, so the next pass diffs rather than assumes.

The shape as specified: one `[[worktree]]` array in the root `state.toml`, each entry `story`, `path` (root-relative), `branch`, `created_at`. `worktree ensure` appends, idempotent per story; `remove` deletes; `list` reads. Discovery inside a registered worktree resolves `.devforgeai/` at the main checkout through `git rev-parse --git-common-dir`. `Ctx::source_root()` is the entry whose `story` equals `[active].build`, and the project root when none matches; it is what the command-running and source-path gate kinds use, with `story files --check`, `antipattern scan`, `commit`, and the PreToolUse scope checks. Documents always come from the root.

Two arms resolve a story from the worktree rather than from `[active].build`, and they are what keep parallel builds working: the PreToolUse `story files --check` arm judges a path under a registered worktree against that entry's `story`, so a write in `../wt/STORY-017` is checked against STORY-017 while `[active].build` names STORY-014; and `phase set build` from inside a registered worktree sets `[active].build` to that worktree's story.

Written in `01-cli` (state table, the state prose, `worktree` subcommands, `## Hooks`, decision 73), `06-build` (steps 4 and 5, decisions 14 and 27), and conventions §7. The old copy-the-state-file claim is gone from every spec — `grep '[build].worktree'` returns nothing outside this file.

**2. `questions.md` Q-023.** The skill-name collision the eval measured: a user-level or plugin skill whose slash name equals one of ours shadows ours for the `Skill` tool, `design` being the one that collided. Recorded with the property that makes it dangerous — the shadowed skill *runs and returns something*, so the failure is quiet rather than an error. The runner now installs co-invoked skills; in a real project the fix is the user's; and the `init` warning over `~/.claude/skills/<name>` and the installed plugins is recorded as a proposal with the reason it is a warning rather than a refusal. The rename escape hatch cross-references Q-021.
